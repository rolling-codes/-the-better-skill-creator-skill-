...
- Does a Red Flags table exist? If missing, run a condensed version of Gate 1 (ask the user for three ways an agent could rationalize skipping the important step) and add it.
- If a Red Flags table exists, check whether any row is generic/placeholder-sounding rather than grounded in an actual failure — generic rows are close to worthless and should be replaced if the user can supply real ones.
