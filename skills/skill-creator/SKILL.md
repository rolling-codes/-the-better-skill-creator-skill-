...
- `scripts/dependency_graph.py <skill-root>` — optional. Build and inspect the dependency graph for a skill root directory (cycle detection, missing-node audit, impact analysis); it takes the skill root as its positional argument, with `--format json|dot|summary`. The workflow and packaging pipeline do not depend on it or on any external graph tooling — reach for it only when you want to inspect a skill's structure by hand.
